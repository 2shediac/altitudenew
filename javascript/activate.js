jQuery(document).ready(function($) {
    ...
    /**
     * Place custom menu either in header or slideout side bar
     * depending on display width. Manage editing state body
     * class. Also toggles off an initial hidden class intended
     * to prevent flashes of unstyled content in the header.
     * Called on page load and on resize.
     */
    function placecustommenu() {
        var headermenu = $('header nav .nav.altitude-custom-menu');
        var sidebarmenu = $('.altitude-menu-sidebar .altitude-custom-menu');
        var sidepanelbutton = $('#side-panel-button');
        var body = $('body');
        var space = $('header.navbar .container-fluid').width() -
                $('#logo').outerWidth(true) -
                $('#side-panel-button').outerWidth(true);
        var menuwidth = headermenu.outerWidth(true);
        if (space >= menuwidth){
            // Show menu in header.
            headermenu.removeClass('hide-menu');
            sidebarmenu.addClass('hide-menu');
            sidepanelbutton.removeClass('show');
            // If the sidebar block region is empty,
            // then update the body class.
            if ($('#sidebar-block #block-region-sidebar-block div.block').length <= 0) {
                body.addClass('empty-region-sidebar-block');
            }
        } else {
            // Show menu in sidebar.
            headermenu.addClass('hide-menu');
            sidebarmenu.removeClass('hide-menu');
            sidepanelbutton.addClass('show');
            // Sidebar block region is now not empty, so add the class.
            body.removeClass('empty-region-sidebar-block');
        }
        headermenu.removeClass('vis-hidden');
        sidebarmenu.removeClass('vis-hidden');
    }
    $(window).resize(placecustommenu);
    placecustommenu();
